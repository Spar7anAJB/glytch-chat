-- 20260213191000_default_members_role_name.sql
begin;

-- If a Glytch already has members, move @members assignments into members.
...
join public.glytch_roles old_role
 ...
 and old_role.name = '@members'
join public.glytch_roles new_role
  on new_role.glytch_id = old_role.glytch_id
 and new_role.name = 'members'
on conflict do nothing;

-- Preserve per-channel overrides by carrying @members channel permissions into members.
...
-- Rename @members role when members does not yet exist.
update public.glytch_roles old_role
set name = 'members'
where old_role.name = '@members'
  and not exists (
    select 1
...
-- Remove leftover @members roles where members already exists.
...
-- Ensure every Glytch has a members role.
...
-- Make sure all members are assigned members role.
insert into public.glytch_member_roles (glytch_id, user_id, role_id)
